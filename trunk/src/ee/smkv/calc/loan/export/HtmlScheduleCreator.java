    ...
    public void appendHtmlScheduleTable(StringBuilder sb) {
        ...
        if(hasDisposableCommission || hasDownPayment){
            ...
            sb.append("<tr class=\"")
                    ...
                    .append("</td><td>")
                    .append(hasDownPayment ? loan.getDownPaymentPayment().setScale(2, mode).toPlainString() : "")
                    .append("</td><td></td><td>");
            if(hasDisposableCommission){
                amount = amount.add(loan.getDisposableCommissionPayment());
                sb
                    .append(loan.getDisposableCommissionPayment().setScale(2, mode).toPlainString())
                    .append("</td><td>");
            }
            ...
            sb
                    .append(amount.setScale(2, mode).toPlainString())
                    .append("</td></tr>");
        }
        ...
        for (Payment payment : loan.getPayments()) {
            ...
            if(hasDisposableCommission || hasMonthlyCommission){
             sb
                .append(payment.getCommission().setScale(2, mode).toPlainString())
                .append("</td><td>");
            }
            sb
        ...
        if(hasDisposableCommission || hasMonthlyCommission){
             sb
                .append(loan.getCommissionsTotal().setScale(2, mode).toPlainString())
                .append("</th><th>");
            }
        sb
